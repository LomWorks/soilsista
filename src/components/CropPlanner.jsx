...
export default function CropPlanner({ userData }) {
  ...
  const savePlan = async () => {
    ...
    try {
      ...
      // Keep user's currentCrops and stats in sync with their plans
      const cropEntry = schedule.crop + (selectedCrop?.variant ? ` (${selectedCrop.variant})` : "");
      await updateDoc(doc(db, "users", userData.userId), {
        currentCrops: arrayUnion(cropEntry),
